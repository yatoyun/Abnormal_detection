...
class Logger(object):
    ...
    def image_summary(self, tag, images, step):
        ...
        for i, img in enumerate(images):
            ...
            #print(img.shape
            img = np.clip(img, 0, 1)
            #scipy toimage is not implemented on version >= 1.3
            #so I port the part of the original function
            img_convert.toimage(img).save(s, format="png")

            #scipy.misc.toimage(img).save(s, format="png")

            # Create an Image object
            img_sum = tf.compat.v1.summary.Summary.Image(encoded_image_string=s.getvalue(),
                                       height=img.shape[0],
                                       width=img.shape[1])
