...
def eval_video(data_path, res_path, is_show=True):
    ...
    fpr, tpr, thresholds = skmetr.roc_curve(
        np.array(gt_labels_list), np.array(res_prob_list), pos_label=2)
    auc = skmetr.auc(fpr, tpr)
    print(('auc:%f' % auc))

    # output_path = os.path.join(res_path,)
    output_path = res_path
    sio.savemat(os.path.join(output_path, video_name + '_gt_label.mat'),
                {'gt_labels_list': np.double(gt_labels_res)})
    sio.savemat(os.path.join(output_path, video_name + '_est_label.mat'),
                {'est_labels_list': np.double(res_prob_list)})
    ...
    if(is_show):
        print('plot show')
        plt.figure()
        plt.plot(gt_labels_list-1)
        plt.plot(res_prob_list)
        plt.savefig(os.path.join(output_path, video_name + '_label.png'))
